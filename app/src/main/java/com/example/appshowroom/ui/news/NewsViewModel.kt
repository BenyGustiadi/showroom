...
import com.example.appshowroom.data.repository.NewsRepositiry
import kotlinx.coroutines.launch

class NewsViewModel : ViewModel() {
    private val repo: NewsRepositiry by lazy { NewsRepositiry() }

    val loading = MutableLiveData(false)
    val actionState = MutableLiveData<ActionState<*>>()

    val listResp = MutableLiveData<List<News>>()
    val detailsResp = MutableLiveData<News>()
